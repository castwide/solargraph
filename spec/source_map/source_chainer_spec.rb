describe Solargraph::SourceMap::SourceChainer do
  ...
  it "recognizes literal strings" do
    ...
    expect(cursor.chain).not_to be_a(Solargraph::SourceMap::Chain::Literal)
    cursor = map.cursor_at(Solargraph::Position.new(0, 1))
    expect(cursor.chain.links.first).to be_a(Solargraph::SourceMap::Chain::Literal)
  ...
  it "recognizes class variables" do
    ...
    cursor = map.cursor_at(Solargraph::Position.new(0, 0))
    expect(cursor.chain.links.first).to be_a(Solargraph::SourceMap::Chain::ClassVariable)
  ...
  it "recognizes instance variables" do
    map = Solargraph::SourceMap.load_string('@foo')
    cursor = map.cursor_at(Solargraph::Position.new(0, 0))
    expect(cursor.chain.links.first).to be_a(Solargraph::SourceMap::Chain::InstanceVariable)
    expect(cursor.chain.links.first.word).to eq('@foo')
  end
  ...
  it "recognizes global variables" do
    map = Solargraph::SourceMap.load_string('$foo')
    cursor = map.cursor_at(Solargraph::Position.new(0, 0))
    expect(cursor.chain.links.first).to be_a(Solargraph::SourceMap::Chain::GlobalVariable)
    expect(cursor.chain.links.first.word).to eq('$foo')
  end
  ...
  it "recognizes unfinished calls" do
    ...
    expect(cursor.chain.links.last).to be_a(Solargraph::SourceMap::Chain::Call)
