describe Solargraph::SourceMap::Chain do
  it "gets empty definitions for undefined links" do
    chain = described_class.new([Solargraph::SourceMap::Chain::Link.new])
  ...
  it "defines constants from core classes" do
    ...
    chain = described_class.new([Solargraph::SourceMap::Chain::Constant.new('String')])
  ...
  it "infers types from core methods" do
    api_map = Solargraph::ApiMap.new
    chain = described_class.new([Solargraph::SourceMap::Chain::Constant.new('String'), Solargraph::SourceMap::Chain::Call.new('new')])
    type = chain.infer(api_map, Solargraph::ComplexType::ROOT, [])
    expect(type.namespace).to eq('String')
  ...
  it "recognizes literals" do
    chain = described_class.new([Solargraph::SourceMap::Chain::Literal.new('String')])
  ...
  it "recognizes unfinished constants" do
    chain = described_class.new([Solargraph::SourceMap::Chain::Constant.new('String'), Solargraph::SourceMap::Chain::Constant.new('<undefined>')])
    expect(chain.constant?).to be(true)
