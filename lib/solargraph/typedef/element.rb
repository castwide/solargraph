...
module Solargraph
  module Typedef
    # Shared abstract interface for the two composable kinds of type data in
    # the Typedef library: a single type token (Type, including its Tuple
    # subclass) and a collection of such tokens (Typeset, a union). A future
    # third kind (e.g. an intersection) subclasses this and works at every
    # call site already written against it, instead of duck-typing the same
    # method names by convention.
    #
    # Path and Token sit outside this hierarchy - they're the leaf tokens a
    # Type's base/params are built from, with their own narrower protocol -
    # but they implement #any_generic? for the same reason: Type's own
    # methods recurse into whatever base/params hold.
    #
    # @abstract Every method below is a stub with no implementation; calling
    #   one directly on Element raises NoMethodError. Subclasses (and only
    ...
    #     its union branches (`types`); for a Type they're `base` and
    #     `params`, but that's an internal detail already folded into a
    #     single fact, so a Type is one member from this question's
